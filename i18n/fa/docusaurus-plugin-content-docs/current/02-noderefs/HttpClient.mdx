...
`HttpClient` برای هر line اصلی یک شناسه جفت‌سازی ۱۲۸ بیتی تازه با CSPRNG از دو مقدار `fastRand64()` (۱۶ بایت از استریم ChaCha20 همان thread) تولید می‌کند و آن را دقیقاً به‌صورت ۳۲ نویسه هگزادسیمال کوچک می‌فرستد. این مقدار metadata معمولی برای تطبیق بخش‌های upload و download است. اگر استقرار به محافظت از ترافیک نیاز دارد، می‌توان `TlsClient`، `EncryptionClient` یا یک نود امنیتی دیگر را به زنجیره اضافه کرد.
