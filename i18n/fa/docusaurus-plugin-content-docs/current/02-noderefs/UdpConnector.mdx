...
| وضعیت صف | رفتار |
| --- | --- |
| رسیدن حجم یا هزینهٔ ظرفیت صف به `1 KiB` | نود قبلی pause می‌شود. |
| تکمیل write‌های pending | نود قبلی resume می‌شود. |
| عبور حجم یا هزینهٔ ظرفیت صف از `16 MiB` | UDP line بسته می‌شود. |
...
صف‌های initialization و DNS هر مقصد، یک بودجهٔ مشترک برای هر line دارند.
عبور از سقف فقط همان association را می‌بندد و associationهای دیگر روی socket مشترک قابل استفاده می‌مانند.
ارسال datagram سیاست فعلی ارسال یا حذف را حفظ می‌کند و صف retry ندارد.
...
این رفتار balancing بر اساس connection یا packet روی line عادی، صف‌های pause و DNS و ارسال زمان بستن را پوشش می‌دهد. بازنشستگی socket پیش از callbackها bindingهای peer را حذف می‌کند و اگر مالک مبدأ line را ببندد، drain متوقف می‌شود. هنگام ارسال زمان بستن، Finish دریافت‌شده به فرستنده منعکس نمی‌شود.
