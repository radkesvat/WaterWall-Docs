...
| downstream `Pause` / `Resume` والد | خروجی parent را بی‌درنگ متوقف می‌کند / FIFO را تخلیه می‌کند؛ نویسندهٔ مسدود به‌صورت محلی و همهٔ childها در آستانهٔ تجمیعی محدود می‌شوند. |
...
- `FlowPause` همان لحظه‌ای ارسال می‌شود که Callback محلی child را Pause می‌کند و منتظر رسیدن طول صف به آستانه‌ای نمی‌ماند. اگر Pause پیش از `Open` رخ دهد، پس از پذیرش `Open` و تمام اولین payload، از جمله batch بزرگ splice، وضعیت child دوباره بررسی می‌شود؛ فقط اگر هنوز متصل و Pause باشد، `FlowPause` ارسال می‌شود. Resume یا بسته‌شدن child در این فاصله مانع ارسال کنترل کهنه می‌شود.
