...
| --- | --- | --- | --- |
| `child-buffer-limit` | integer | `8388608` | بیشترین تعداد بایت صف‌شده برای یک child در حالت pause. باید بزرگ‌تر از `0` باشد. با رسیدن به سقف، child با فریم `Close` بسته می‌شود. |
...
پیاده‌سازی payloadهای بزرگ‌تر از `0xFFFF - 8` را رد می‌کند؛ بنابراین هر فریم داده MUX حداکثر `65527` بایت payload حمل می‌کند. `MuxClient` یک payload بزرگ child را میان چند فریم تقسیم نمی‌کند و داده ورودی باید از ابتدا در این سقف جا شود.
