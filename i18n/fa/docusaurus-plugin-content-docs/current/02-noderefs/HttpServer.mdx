...
حد انتظار upload در split برابر `131070 * max(1, ceil(L / 32768))` بایت است و فقط عبور از آن موجب بستن می‌شود. `L` مبنای مستقل بودجهٔ انتظار در pool خط است: ۶۴ KiB در S1/S2 و ۱ MiB در پروفایل‌های بالاتر. این مبنا از اندازهٔ بافرهای عادی و سقف splice مستقل است. header و جفت‌شدن ابتدا پردازش می‌شوند؛ body همراه header جزو حد header نیست. گزینهٔ `no-split-upload-buffering-limit` همچنان این حد را غیرفعال می‌کند.
