...
واتروال است. این نود در میانه chain قرار می‌گیرد و برای هر connection یک شاخه
انتخاب می‌کند. ruleهایی که فقط به metadata موجود در `Init` نیاز دارند همان‌جا
انتخاب می‌شوند؛ ruleهای نیازمند sniffing با نخستین upstream payload انتخاب می‌شوند.
...
`Router` را انتخاب‌گر شاخه‌ای در نظر بگیرید که هر وقت ممکن باشد زود تصمیم
می‌گیرد و فقط هنگام نیاز به بررسی bytes تصمیم را به تأخیر می‌اندازد:

- اگر هیچ rule یا تنظیم اصلی به inspection payload نیاز نداشته باشد، در `Init`
  بر پایه metadata مبدأ/مقصد، نوع شبکه و credentialها شاخه را انتخاب و همان‌جا
  مقداردهی اولیه می‌کند.
- ruleی که به تشخیص protocol، HTTP Host/:authority، TLS/QUIC SNI یا HTTP
  `Upgrade` نیاز دارد منتظر نخستین upstream payload می‌ماند و آن bytes را تا
  زمان تصمیم‌گیری در buffer نگه می‌دارد.
- پس از match شدن یک rule تأخیری، شاخه انتخابی را مقداردهی اولیه می‌کند و
  bytes bufferشده را روی همان شاخه دوباره پخش می‌کند.
...
:::warning
ruleهای نیازمند sniffing همچنان باید ابتدا از client داده بگیرند. برای
protocolهای server-first یا clientهایی که پیش از نخستین write به downstream
establishment نیاز دارند، فقط از ruleهای metadata-only (برای نمونه `network`،
port، address یا credential) استفاده کنید و `sniffing`، `protocol` و
`attributes` را در آن Router نیاورید.
:::
...
| callback | رفتار |
| --- | --- |
| upstream `Init` | state داخلی را مقداردهی اولیه و فقط بیت‌های protocol تشخیص‌داده‌شده را از `dest_ctx.optional_flags` پاک می‌کند؛ metadata کنترل مسیر یک‌بارمصرف حفظ می‌شود. اگر هیچ rule یا تنظیمی به sniffing payload نیاز نداشته باشد، شاخه انتخابی همین‌جا آماده می‌شود. |
...
`Router` عمداً در upstream `Init` فقط زیرمجموعه بیت‌های protocol تشخیص‌داده‌شده در `dest_ctx.optional_flags` را پاک می‌کند تا این بیت‌ها از یک `Router` به `Router` بعدی روی همان line منتقل نشوند. بیت‌های جداگانه کنترل مسیر که tunnelهای protocol برای یک adapter بعدی حمل می‌کنند حفظ می‌شوند و هرگز `protocol` match محسوب نمی‌شوند.
...
وقتی ruleها فقط metadata موجود روی line را می‌خواهند، `Router` شاخه را در `Init` آماده می‌کند. اگر هر rule یا تنظیم اصلی به sniffing payload نیاز داشته باشد، تصمیم تا نخستین upstream payload عقب می‌افتد؛ چنین پیکربندی‌ای تا رسیدن داده client نمی‌تواند شاخه server-first را مقداردهی اولیه کند.
