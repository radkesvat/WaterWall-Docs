...
description: "مستندات مرجع نود SniffRouter."
---
...
- اگر payload اول فقط prefix handshake باشد، SniffRouter warning log می‌کند و
...
SniffRouter byteهای اولیه upstream را تا وقتی بتواند connection را classify کند buffer می‌کند.
...
Reverse detection سخت‌گیرانه‌تر است: اگر payload اول فقط prefix ناقص reverse-handshake باشد، منتظر byteهای بیشتر نمی‌ماند و به top-level `next` fallback می‌کند.

بعد از انتخاب route، payloadهای بعدی مستقیم به target انتخاب‌شده یا default `next` forward می‌شوند. connection دوباره classify نمی‌شود.
...
- target branch نباید قبلاً به previous ناسازگار bind شده باشد
- target می‌تواند همان node مربوط به top-level `next` باشد

وقتی branch جای دیگری در config قرار دارد از `Bridge` target استفاده کنید.
...
| upstream `Est` | فقط به default branch forward می‌شود. route targetها اغلب adapter/server node هستند که upstream `Est` برای آن‌ها معتبر نیست. |
...
| downstream `Finish` | state داخلی را قبل از forward کردن finish به previous node destroy می‌کند. |
