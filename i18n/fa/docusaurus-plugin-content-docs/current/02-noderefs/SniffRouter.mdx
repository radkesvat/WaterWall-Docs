...
با حذف `detection`، هر دو روش HTTP/1 Host و TLS ClientHello SNI برای آن route
فعال می‌شوند. فقط وقتی route باید به یکی از این دو روش محدود شود، مقدار
`"http1"` یا `"tls"` را صریح بنویسید.
...
| --- | --- | --- | --- |
| `domains` | string یا array | بله | الگوهای domain برای تشخیص HTTP/1 Host و TLS SNI. |
| `domain` | string | نه | alias برای یک domain. با `domains` هم‌زمان استفاده نشود. |
| `detection` | string یا array | نه | حذف آن یعنی هر دو مقدار `["http1", "tls"]`. |
...
| مقدار | معنی |
| --- | --- |
| `http1` | خواندن HTTP/1 Host؛ به‌صورت پیش‌فرض فعال است. |
| `tls` | خواندن TLS ClientHello SNI؛ به‌صورت پیش‌فرض فعال است. |
...
اگر فیلد `detection` حذف شود، تشخیص HTTP/1 Host برای route فعال است:
...
اگر فیلد `detection` حذف شود، تشخیص TLS ClientHello SNI نیز فعال است:
...
وقتی این route باید فقط TLS را بررسی کند و HTTP/1 Host را نپذیرد، مقدار
`"detection": "tls"` را صریح بنویسید.
...
قوانین دقیق دسته‌بندی و بافرینگ:
- پیام‌های کامل ClientHello ابتدا ارزیابی می‌شوند، حتی اگر اندازه کل پیام بزرگتر از ۸۱۹۲ بایت باشد.
- اگر ورودی هنوز ناقص باشد و مجموع بایت‌های دریافت شده به سقف ۸۱۹۲ بایت برسد، `SniffRouter` انتظار را متوقف کرده و شاخه پیش‌فرض `next` را انتخاب می‌کند.
- پیام‌های ناقصی که کمتر از ۸۱۹۲ بایت دارند در حالت تصمیم‌گیری‌نشده باقی می‌مانند؛ `SniffRouter` تایمر اختصاصی ندارد و timeoutهای listener اولیه (مانند `TcpListener`) مهلت اتصال را تعیین می‌کنند.
- بایت‌های اولیه بافرشده به صورت یک جریان دست‌نخورده با ترتیب FIFO به شاخه انتخابی ارسال می‌شوند (مرزهای بسته یا callback لزوماً حفظ نمی‌شوند).
...
## مسیریابی TLS SNI و استتار Nginx

نود `SniffRouter` می‌تواند **پیش از** `TlsServer` قرار گیرد تا مقدار SNI در TLS ClientHello را بررسی کند و دامنه‌های منطبق را به مسیر خاتمه TLS محافظت‌شده بفرستد، در حالی که سایر اتصالات را به یک سرور پوششی واقعی (مانند Nginx HTTPS) هدایت کند.
...
- **تطابق SNI**: اتصالاتی که TLS ClientHello معتبر با SNI منطبق ارائه دهند به `protected-tls-server` هدایت می‌شوند.
- **مسیر Fallback پیش‌فرض**: اتصالاتی با SNI نامنطبق، فاقد SNI، ورودی نامعتبر یا ترافیک Plaintext HTTP همگی به `next` در سطح اصلی (`nginx-fallback-connector`) فرستاده می‌شوند.
- **چرا Fallback واقعی TLS در Nginx**: هدایت ترافیک پیش‌فرض به سرور واقعی Nginx تضمین می‌کند که handshakeهای با SNI ناشناخته، بدون SNI و درخواست‌های plaintext پاسخ‌های معتبر و طبیعی Nginx (مانند خطای 400 "The plain HTTP request was sent to HTTPS port") دریافت کنند و از ایجاد اثر انگشت مصنوعی در رد handshake جلوگیری شود.
...
## پیشرفته: مسیریابی Handshake تونل Reverse

این قابلیت فقط برای topology یک reverse tunnel واقعی است که نودهای
`ReverseClient` و `ReverseServer` را دارد. این گزینه یک روش عمومی برای برعکس
کردن جهت traffic یا callbackهای WaterWall نیست. در این سناریو، `ReverseClient`
لینک‌های outbound آماده می‌سازد و handshake خصوصی را در ابتدای هر لینک قرار
می‌دهد؛ chainهای معمولی که این نودها را ندارند چنین handshakeای تولید نمی‌کنند
و نباید این detector را تنظیم کنند.

این روش زمانی کاربرد دارد که یک TLS entrypoint رمزگشایی‌شده هم لینک‌های آماده
تونل و هم سایت camouflage با SNI یکسان را حمل می‌کند:
...
تنظیمات پیشرفته مربوط به این قابلیت:

| گزینه | نوع | پیش‌فرض | توضیح |
| --- | --- | --- | --- |
| `reverse-secret-length` | integer | `640` | طول handshake خصوصی؛ بازه معتبر `1..1024`. |
| `reverse-secret` | ASCII string | unset | secret مربوط به XOR برای ساخت handshake خصوصی؛ باید در هر دو نود peer یکسان باشد. |

در route از `"detection": "reverse"` استفاده کنید. مقدارهای `"reverse-tls"`
و `"reverse-handshake"` alias هستند. routeای که فقط از این detector استفاده
می‌کند به `domain` یا `domains` نیاز ندارد و الگوهای domain موجود را نادیده
می‌گیرد. اگر همین route با `http1` یا `tls` ترکیب شود، domainها برای آن روش‌های
مبتنی بر domain همچنان ضروری‌اند.
...
signature پیش‌فرض ۶۴۰ بایت `0xFF` است. `SniffRouter` مقدار سفارشی را دقیقاً با
همان الگوریتم `ReverseClient` و `ReverseServer` می‌سازد، بنابراین تنظیمات هر سه
نود باید یکسان باشند. SniffRouter را بعد از TLS termination قرار دهید تا
handshake از بایت صفر stream رمزگشایی‌شده شروع شود. SniffRouter فقط بایت‌ها را
می‌بیند و دست‌نخورده بازپخش می‌کند؛ `ReverseServer` دوباره آن‌ها را اعتبارسنجی
و حذف می‌کند. prefix ناقص در payload اول یک warning ثبت می‌کند و فوراً به
top-level `next` می‌رود. همچنین header مربوط به PROXY protocol باید پیش از
SniffRouter حذف شود. `Router` در حال حاضر این detector را ندارد.
