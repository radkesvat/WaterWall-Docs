...
هر connection را بررسی می‌کند، بر اساس بایت‌های قابل مشاهده تصمیم می‌گیرد و
تمام connection را از نخستین route مناسب عبور می‌دهد.

این نود از `Router` ساده‌تر و تخصصی‌تر است. اگر فقط به routing بر پایه HTTP/1
Host، TLS ClientHello SNI یا handshake داخلی `ReverseClient`/`ReverseServer`
نیاز دارید، `SniffRouter` انتخاب مناسبی است. برای شرط‌هایی مانند source IP،
...
- یک انتخاب‌گر شاخه کوچک و صرفاً مبتنی بر content
