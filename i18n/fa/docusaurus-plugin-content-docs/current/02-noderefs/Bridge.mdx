...
description: "مستندات مرجع نود Bridge."
---
...
`Est` هم مانند بقیه callbackهای lifecycle در همان مدل relay می‌شود.
...
`Bridge` هیچ object از نوع `line_t` نمی‌سازد و destroy نمی‌کند. همان pointer مربوط به line را از طریق Bridge جفت forward
می‌کند و per-line state آن عملاً استفاده خاصی ندارد.

رفتار payload هم relay مستقیم است:

| جهت ورود به Bridge فعلی | Forward شدن به عنوان |
| --- | --- |
...
چون Bridge هیچ byteی prepend نمی‌کند و framing ندارد، `required_padding_left = 0` است.
...
- packet/stream converter نیست؛ فقط callbackهای WaterWall را بین دو نقطه relay
  می‌کند.
- Bridge جفت باید در همان config file و زیر همان node manager configuration وجود داشته باشد.
