...
در گیرنده، `PacketReceiver.protocol-number` را برابر protocolهای مورد انتظار و `PacketReceiver.expected-packets-per-ip` را در همه حالت‌ها، از جمله `ALL`، برابر `PacketSender.packets-per-ip` بگذارید. گیرنده مجموع هر IP را خودش محاسبه می‌کند و protocolها را جداگانه گزارش می‌دهد. برای چند protocol می‌توان از آرایه‌هایی مانند `[6, 17]` یا `["TCP", "UDP"]` استفاده کرد.
