...
رویداد transport `Est` بلافاصله و فقط یک بار منتقل می‌شود و به آمادگی TLS یا Reality وابسته نیست.
TlsClient داخلی callback اختصاصی پایان handshake را ثبت می‌کند؛ ثبت binding و شروع takeover تنها در این callback
انجام می‌شود. پایان TLS و فعال شدن Reality هیچ Est دیگری تولید نمی‌کنند. payload پس از Init و پیش از Est معتبر است.
صف ضروری plaintext اولیه و ترتیب ورودی، با احتساب head فعال، حداکثر ۲ MiB و ۱٬۰۲۴ buffer دارد؛ برابری پذیرفته می‌شود.
پذیرش صف تراکنشی است و سرریز، line قرض‌گرفته‌شده را از طریق owner می‌بندد. پایان handshake و Resume صف قدیمی را
با ترتیب FIFO و فقط بدون Pause تخلیه می‌کنند. Pause در callback ارسال record بعدی را متوقف می‌کند و ورودی
re-entrant از head قدیمی سبقت نمی‌گیرد. داده کاربردی پیش از ACK ارسال نمی‌شود.
نخستین plaintext نگه‌داشته‌شده پیش از Pause کردن source در صف ثبت می‌شود. انتظار protocol و ترتیب با فشار wire
یک latch اعلان مشترک دارند؛ Resume تنها پس از فعال‌سازی احرازشده، خالی شدن FIFO قدیمی و رفع فشار wire صادر می‌شود.
دریافت recordهای TLS و handoff در این مدت ادامه دارد.
