...
`PingServer` packet tunnel سمت server و جفت `PingClient` است. مثل client، payloadهای packet در upstream را به traffic شبیه ICMP disguise می‌کند و traffic matching سمت peer را در downstream reverse می‌کند.
...
`PingServer` همان callback shape مربوط به payload را مثل `PingClient` استفاده می‌کند:

| جهت | رفتار |
| --- | --- |
| upstream `Payload` | packetهای plain را encapsulate یا rewrite می‌کند، سپس با `tunnelNextUpStreamPayload()` forward می‌کند. |
| downstream `Payload` | traffic matching سمت peer را decapsulate یا restore می‌کند، سپس با `tunnelPrevDownStreamPayload()` forward می‌کند. |
