...
هنگام پردازش payloadِ upstream، نود هنگام فرستادن مجموعه junk با `lineRef()` یک Reference موقت از line نگه می‌دارد، سپس `lineIsAlive()` را بررسی و Reference را با `lineUnref()` آزاد می‌کند. این Reference فقط تخصیص حافظه را نگه می‌دارد و قفل یا دسترسی میان‌Worker ایجاد نمی‌کند. ارسال کپی‌های با تأخیر با `lineScheduleDelayedTaskWithBuf()` زمان‌بندی می‌شود؛ بنابراین scheduler یک reference موقت از line نگه می‌دارد و اگر line پیش از اجرای timer بسته شده باشد، بافر کپی را آزاد می‌کند.
