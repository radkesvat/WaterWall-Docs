...
این نود هنگام ساخت زنجیره (chain construction)، نود شاخه `down` خود را به‌عنوان نود `next` متصل (bind) می‌کند تا بتواند بسته‌ها (packetها) را در جهت downstream از شاخه `down` دریافت کند. به همین دلیل `can_have_next` برابر `true` و `layer_group_next_node` برابر `kNodeLayer3` تنظیم شده است.
