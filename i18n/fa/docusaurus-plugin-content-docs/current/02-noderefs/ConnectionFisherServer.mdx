...
مسیر ادامه پیش از فراخوانی Init بعدی منتشر می‌شود. Est واقعی ترنسپورت، حتی اگر از داخل Init برسد، فوری و فقط یک بار منتقل می‌شود. payload برای Est منتظر نمی‌ماند.

بدنه همراه FISH? و نخستین payload پس از probe، هنگام پاسخ FISH!، Init بعدی و Est
بازدرآمده جای FIFO خود را حفظ می‌کنند. ترتیب موقت ورودی، شامل بدنه در انتظار بازگشت
Init، سقف 1 MiB دارد و در همان dispatch پذیرفته‌شده تکمیل می‌شود. Pause یا Est به‌تنهایی
صف داده آماده ایجاد نمی‌کنند. حد ۴۰۹۶ بایتی مونتاژ probe ثابت است. بسته‌شدن در Init یا
Est داده محلی قدیمی را آزاد می‌کند.
