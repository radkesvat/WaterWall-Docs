...
reserve حافظه از مقادیر بایت جدول بالا استفاده می‌کند. سقف مستقل
`memory-fallback-max-live-children` مجموع childهای زنده همه parentها و workerهای
یک نمونه MuxServer را محدود می‌کند و پیش‌فرض‌های آن به این صورت است:

| پروفایل RAM | سقف پیش‌فرض fallback live-child |
| --- | ---: |
...
| L1 | `52095` |
| L2 (`server`، پیش‌فرض سراسری) | `60000` |
