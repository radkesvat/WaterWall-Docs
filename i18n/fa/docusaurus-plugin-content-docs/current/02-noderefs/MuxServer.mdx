...
آمار شامل worker id، وضعیت write-pause والد، مجموع charge نگه‌داشته‌شده، تعداد child، تعداد childهای منتظر تکمیل ترتیبی `Close`، تعداد childهای read-pause و تعداد childهای write-pause است. فیلد قدیمی `parent-line-read-paused` برای سازگاری همچنان با مقدار `no` در لاگ باقی می‌ماند و `parent-child-queue-charge` و `parent-input-queue-charge` charge سقف سخت را گزارش می‌کند، نه طول منطقی payload.
