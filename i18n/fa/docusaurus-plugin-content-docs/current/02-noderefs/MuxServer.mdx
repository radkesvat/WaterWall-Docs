...
| `parent-buffer-limit` | integer | `50331648` | سقف charge تخصیص‌های نگه‌داشته‌شده در صف childها به‌ازای هر parent. با رسیدن به سقف، child با بیشترین charge بسته می‌شود؛ در صورت مساوی بودن، اولویت با قدیمی‌ترین child متصل است. برای غیرفعال کردن سقف تجمیعی مقدار `0` را قرار دهید. |
...
هر فریم `Data` حداکثر `65,527` بایت payload حمل می‌کند. encoder مشترک، payload
...
| upstream `Pause` / `Resume` والد | خروجی parent را بی‌درنگ متوقف می‌کند / FIFO را تخلیه می‌کند؛ محدودیت childها فقط در threshold صف اعمال می‌شود. |
...
اختیاری زیر برحسب بایتِ charge تخصیص `sbuf_t` هستند: ظرفیت کامل با padding،
هدر بافر و سربار alignment را حساب می‌کنند، نه طول منطقی payload یا RSS فرایند.
...
این بودجه برای هر parent جداگانه اعمال می‌شود. `parent-buffer-limit` همچنان
صف داده ورودیِ decodeشده برای childها را محدود می‌کند و معنی آن تغییر نکرده است.
...
بافر دریافت parent یک جریان بایت است و می‌تواند بافرهای transport را با هم
ادغام کند. decoder در هر بار دقیقاً یک هدر MUX و payload به طول اعلام‌شده را
می‌خواند. برای child باز، هر فریم `Data` به یک callback از نوع `Payload`
تبدیل می‌شود. صف child در حالت pause این فریم‌های استخراج‌شده را به‌صورت
ورودی‌های جدا نگه می‌دارد و پس از Resume به‌ترتیب FIFO تخلیه می‌کند.
...
اصلی UDP را. هر دو encoder یک payload تا سقف `65,527` بایت را در یک فریم
...
صف فرزند Pauseشده از بافرهای small یا medium با ظرفیت ثابت ۶۴ KiB استفاده می‌کند.
فریم فقط وقتی به یک بافر کوچک‌تر از pool کپی می‌شود که payload و padding در آن جا
شوند و ظرفیت نگه‌داری‌شده کاهش یابد؛ متادیتای lifetime نیز منتقل می‌شود. فرزند
بدون Pause همان بافر decodeشده را مستقیم می‌گیرد. محدودیت صف همچنان هزینهٔ واقعی
allocation را حساب می‌کند و کنترل جریان برحسب طول payload است. بافر medium برای
کارهای کمکی است؛ خواندن عادی event loop از بافر large استفاده می‌کند.
