...
| حداکثر بایت در بافر خواندن | `max(65536, 2 * L) + 65553` | `L` ظرفیت payload بافر بزرگ pool خط است. |
...
هر ورودی در یک بافر شامل چند فریم ارسال می‌شود. گیرنده header و body تکه‌تکه را بازسازی می‌کند و فریم‌های کامل را با یک callback می‌فرستد تا پس از Pause ارسال دیگری آغاز نشود. رکورد نامعتبر باعث بسته‌شدن خط از مسیر مالک آن می‌شود.
