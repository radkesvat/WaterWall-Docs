...
جهت upstream را wrap می‌کند و `PingServer` آن را در callback جهت upstream بازیابی
و به نود بعدی می‌فرستد. در مسیر برگشت، `PingServer`، ‏response جهت downstream را
wrap می‌کند و `PingClient` آن را در callback جهت downstream بازیابی می‌کند.
...
client تغییری نمی‌کند، اما chain قدیمی
`TunDevice -> PingServer -> RawSocket` باید به
`RawSocket -> PingServer -> TunDevice` تغییر کند. topologyهای Ping مبتنی بر
Bridge جفت‌شده را با اتصال مستقیم `PingClient -> PingServer` جایگزین کنید.
پیاده‌سازی `PingClient`، ‏settingها و wire format تغییری نکرده‌اند؛ بنابراین
...
نیاز دارد. upgrade هم‌زمان هر دو peer می‌تواند عملیات را ساده‌تر کند، اما الزام
