...
`PingServer` جفت‌شده همین مدل direction payload را استفاده می‌کند: upstream wrap می‌کند و downstream unwrap می‌کند. در topologyهای تست با `Bridge` مطمئن شوید traffic wrap شده از یک سمت به downstream path سمت مقابل برسد.
