...
- packetهای بازیابی‌شده را به سمت قبلی packet برمی‌گرداند.
...
| stream side به packet side | بایت‌های stream در buffer جمع و به packetهای IPv4 decode می‌شوند؛ سپس پس از اعتبارسنجی اختیاری، در جهت downstream به سمت packet می‌روند. |
