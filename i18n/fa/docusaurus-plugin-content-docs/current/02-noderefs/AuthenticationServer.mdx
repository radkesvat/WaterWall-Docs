...
هر پیام درخواست احرازشده، timestamp آخرین فعالیت session را به‌روز می‌کند. Worker شماره `0` هر `60000` میلی‌ثانیه sessionهای بی‌کار را پاک‌سازی می‌کند. Sessionها فقط state زمان اجرا هستند و هنگام restart ذخیره نمی‌شوند.
