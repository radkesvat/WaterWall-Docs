...
اگر مشخصات ورود با یکی از ورودی‌های `settings.auth-clients` منطبق باشد، سرور یک session می‌سازد و token شصت‌وچهاربایتی برمی‌گرداند. این token از ۳۲ بایت دریافتی مستقیم از منبع تصادفی امن سیستم‌عامل ساخته و به‌صورت ۶۴ بایت hexadecimal با حروف کوچک encode می‌شود.
