...
`UdpStatelessSocket` فقط در ابتدای chain پشتیبانی می‌شود؛ هنگام استفاده از آن، سمت transport وایرگارد به `prev` متصل است.

```text
UdpStatelessSocket -> WireGuardDevice -> TunDevice
```

`WireGuardDevice` می‌تواند سمت transport را در هر یک از دو جهت `next` یا `prev` داشته باشد. این نقش را از لایه‌های حل‌شده مجاور زنجیره استخراج می‌کند: L3 سمت پکت داخلی است و L4 سمت transport بیرونی است.
...
هیچ تنظیم جهت یا جستجوی نوع نودی وجود ندارد. پس از سرهم‌بندی زنجیره، layer solver باید دو لبه مجاور WireGuard را دقیقاً به یک سمت L3 و یک سمت L4 تفکیک کند. لبه L3 سمت پکت داخلی و لبه L4 سمت transport بیرونی می‌شود. زنجیره‌های مبهم به‌جای پیش‌فرض قرار دادن `next`، در راه‌اندازی با خطا مواجه می‌شوند؛ در صورت نیاز در توپولوژی‌های پیچیده، یک پل صریح پکت/استریم مانند `PacketsToStream` یا `StreamToPackets` اضافه کنید.
...
این جهت‌ها منحصراً از نقش‌های حل‌شده لایه‌های مجاور انتخاب می‌شوند.
...
- انتظار اینکه OS WireGuard interface ساخته شود. برای رفتار اینترفیس از `TunDevice` یا شبکه سیستم‌عامل میزبان استفاده کنید.
- مبهم گذاشتن هر دو دامنه لبه مجاور WireGuard به‌جای استفاده از یک پل صریح پکت/استریم.
