...
هسته 1500). بازهٔ معتبر **68..65535 بایت** است: MTU خام IPv4 برای تمام رابط‌های مجازی هر worker، حتی رابط‌هایی که بعداً ساخته می‌شوند.
...
fragmentation موجود IPv4 در lwIP از MTU همین رابط پیروی می‌کند.
...
هر دو checksum header و transport اصلاح می‌شوند. fragment ورودی پیش از تحویل به پشته با خطای fatal رد می‌شود.

Input misaligned به aligned sbuf copy و از همان custom-pbuf wrapper عبور می‌کند.
اندازهٔ poolهای مشترک lwIP تغییر نکرده است؛ TCP out-of-order همچنان می‌تواند
wrapper را نگه دارد و محدودیت per-PCB برقرار است.
...
این الزام روی upstream packet input برقرار است و fake DNS نیز فقط packet کامل
می‌بیند. خروجی netif هر worker از `mtu` همان نمونه استفاده می‌کند و پاسخ بزرگ‌تر را با
`ip4_frag()` fragment می‌کند. خروجی Fake-DNS نیز از `ip4_output_if()` می‌گذرد.
packet کامل پس از reassembly مالکیت معمول buffer دارد و claim عمر fragment همراه
آن منتقل نمی‌شود.
...
pending تحت lifecycle-v2 پیش از stop و destruction component تسویه می‌شوند؛ gate
پذیرش output بسته می‌شود تا work دیررس بدون callback به همسایهٔ متوقف‌شده cancel
شود. Work عادیِ ردشده منابع تحت مالکیت خود را آزاد می‌کند و teardown را به مسیر owner می‌سپارد؛
lineای که Init آن کامل شده است در drain همان owner-worker دقیقاً یک `Finish`
دریافت می‌کند.

Callbackهای lwIP کار را با قرارداد نتیجه‌دار `lineScheduleTask()` و
`lineScheduleTaskWithBuf()` به worker مالک line می‌سپارند. ردشدن control ضروری،
producer را در حالی terminal می‌کند که core lock رجیستری را محافظت می‌کند؛ callback
لغو عمداً `NULL` است تا cleanup دوباره وارد همان lock نشود. تحویل بافر در هر نتیجه
مالکیت کپی را منتقل می‌کند.
...
سقف پذیرش TCP برابر `P + L` است: `P = max-pending-bytes` و `L` حاشیهٔ ثابت
۱ MiB مستقل از اندازهٔ pool، پروفایل حافظه و تنظیمات splice است. این حاشیهٔ محدود برای ورودیِ قبلاً تحویل‌شده، پیش از
اثرگذاری Pause، است. هر رکورد تا ACK کامل طول اصلی payload را حساب می‌کند.
محدودیت تعداد entry مستقل است و پذیرش پیش از انتقال مالکیت به صف بررسی می‌شود.
وقتی این هزینه از `P` بیشتر است، حتی اگر صف write خالی باشد، Pause تا پیشرفت کافی ACK حفظ می‌شود.
