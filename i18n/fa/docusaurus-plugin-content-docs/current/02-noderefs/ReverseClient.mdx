...
## چرا در این ساختار از Bridge استفاده می‌شود
...
## هم‌جواری الزامی در زنجیره

نود `Bridge` باید دقیقاً قبل از `ReverseClient` قرار بگیرد: `Bridge -> ReverseClient`.
هیچ نودی نباید بین این دو قرار بگیرد. در سمت server نیز `Bridge` باید دقیقاً بعد
از `ReverseServer` باشد: `ReverseServer -> Bridge`، بدون هیچ نود واسط.

این‌ها قواعد طراحی زنجیره هستند و کاربر باید هنگام طراحی آن‌ها را رعایت کند.
کد هیچ‌یک از این دو شرط هم‌جواری را به‌صورت خودکار بررسی و اعمال نمی‌کند.
نودهای transport یا پردازش باید بیرون از این دو فاصله قرار بگیرند؛ مسیر outbound
بعد از `ReverseClient` همچنان می‌تواند شامل نودهای transport توضیح‌داده‌شده در ادامه باشد.
...
handshake تولیدشده همچنان بافر عادی است. هنگام عبور دادهٔ برنامه میان دو line عادی تحت مالکیت نود، حتی در فعال‌سازی جفت، بدنه در pipe خصوصی می‌ماند. اگر Init در downstream جفت را ببندد، بافر نگه‌داشته‌شده دقیقاً یک بار بازیافت می‌شود. اتصال مستقیم Bridge -> ReverseClient را حفظ کنید. این قابلیت به معنی پشتیبانی ReverseServer نیست؛ زنجیره‌های دارای ReverseServer همچنان از بافر عادی استفاده می‌کنند.
