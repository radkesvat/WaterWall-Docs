...
ساختار بیرونی recordهای handshake مقصد به‌صورت تدریجی در state محدود parser و tracker دنبال می‌شود؛ کل جریان مقصد پیش از آماده‌شدن keyها هرگز نگه‌داری یا دوباره پردازش نمی‌شود. recordهای رمز‌نشده TLS 1.2 پیش از مشخص‌شدن profile رمز هم قابل دنبال‌کردن هستند. profile انتخاب‌شده به‌محض آماده‌شدن هر دو hello نصب می‌شود، حتی اگر ClientHello پس از ServerHello ثبت‌شده برسد. خطای tracking مربوط به TLS 1.2 پیش از نصب profile، در صورت انتخاب TLS 1.2 باعث fallback به Visitor می‌شود؛ این خطا مذاکره TLS 1.3 را رد نمی‌کند.
