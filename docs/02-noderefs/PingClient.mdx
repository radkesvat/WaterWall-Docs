...
The paired `PingServer` uses the same payload direction model: upstream wraps,
downstream unwraps. In a bridge-style test topology, make sure wrapped traffic
from one side reaches the peer's downstream path.
