...
Sync note: Any change to this file must also be applied to WaterWall/tunnels/PacketsToStream/description.md and WaterWall-Docs/i18n/fa/docusaurus-plugin-content-docs/current/02-noderefs/PacketsToStream.mdx, and all files must keep the same documentation version.
-->
...
- Buffers return stream bytes and reconstructs IPv4 packets from the total
  length field.
- Sends reconstructed return packets back to the previous packet side on the
  worker that owns each packet's inner flow.
...
| stream side back to packet side | Stream bytes are buffered, decoded into IPv4 packets, optionally validated, re-affinitized from the inner tuple, then emitted downstream on that flow's own worker packet line. |
...
7. computes the shared symmetric inner-flow hash
8. sends it back to the previous packet-side node on `packet_line[hash % workers]`,
   directly when that is the current worker and queued to the target worker
   otherwise

The outer stream line's worker is deliberately not used to pick the packet line.
A peer such as `StreamToPackets` selects a return carrier per inner flow out of
its whole active line pool, so any flow can arrive on any of these lines;
re-deriving affinity from the inner tuple is what makes that safe, and what keeps
both directions of a flow on one packet worker. A decoded packet whose flow cannot
be hashed is logged and dropped rather than round-robined, because its stateful
worker ownership cannot be established.

Only the decoded packets are re-affinitized. The parser, the sensitive-mode
heartbeat state, and output-line recreation all stay owned by the stream line's
own worker, and this node keeps one stream output line per packet worker on its
send side.
