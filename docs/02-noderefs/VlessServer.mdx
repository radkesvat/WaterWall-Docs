...
| `fallback-intentional-delay-ms` | `7` | Delay for upstream payloads sent to fallback. Nonzero delay uses configured jitter. At `0`, an active fallback with no older FIFO batch or scheduled drain forwards inline; independently retained bytes remain FIFO and Resume drains them before later payload can overtake. |
...
6. combines the VLESS response header with the first nonempty backend reply in one ordinary buffer
...
UDP frames with length `0` are invalid. UDP parser input is capped at 2,162,689 logical bytes, including cached metadata. Partial two-byte length prefixes and datagram bodies wait for later input.
...
- at delay `0`, an active fallback with no older FIFO batch or scheduled drain forwards inline; independently retained
  bytes remain FIFO and Resume schedules their drain before later payload can overtake them
- the delayed fallback FIFO is bounded to the shared upstream 2 MiB / 1,024-buffer budget
- on upstream `Finish`, queued bytes flush synchronously in FIFO order before fallback `Finish` when fallback can still
  accept payload outside branch Init; a paused or initializing fallback instead discards the still-local batch and closes
...
`VlessServer` prepends the 2-byte response header once and a 2-byte length prefix on every UDP reply. The first UDP
reply needs both, so it advertises:
