...
| `fallback-intentional-delay-ms` | `7` | Delay for upstream payloads sent to fallback. Nonzero delay uses configured jitter. At `0`, accepted input forwards synchronously, including completion under Pause; branch Init and reentry preserve FIFO. Must not be negative. |
...
8192 bytes close the affected Trojan line. Backend replies are wrapped and forwarded synchronously without waiting for Est or Resume.
...
- at delay `0`, admitted input forwards synchronously even under Pause; Init and reentry preserve FIFO
- the delayed fallback FIFO is bounded to 2 MiB and 1,024 buffers per line
