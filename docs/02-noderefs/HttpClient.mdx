...
For each main line, `HttpClient` generates a fresh 128-bit CSPRNG pairing identifier from two `fastRand64()` values (16 bytes from the thread's ChaCha20 stream) and encodes it as exactly 32 lowercase hexadecimal characters. The value is ordinary routing metadata used to match the upload and download halves. Add `TlsClient`, `EncryptionClient`, or another security node if the deployment needs protected transport.
