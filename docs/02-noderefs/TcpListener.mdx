...
The line source context is populated from the accepted connection. In the current
implementation, the peer IP is recorded, and the local port that accepted the
connection is stored in the source context port field.
