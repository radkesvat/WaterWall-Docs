...
description: "Reference documentation for the PacketSender node."
---

<!--
Documentation version: 153
Sync note: Any change to this file must also be applied to WaterWall/tunnels/PacketSender/description.md and WaterWall-Docs/i18n/fa/docusaurus-plugin-content-docs/current/02-noderefs/PacketSender.mdx, and all files must keep the same documentation version.
-->
...
- Stops a worker's timer and send loop if the downstream side finishes that
  worker packet line.
...
Set `PacketReceiver.protocol-number` to the expected protocol selection and
`PacketReceiver.expected-packets-per-ip` to sender `packets-per-ip`, including
in `ALL` mode. PacketReceiver calculates the per-IP total itself and reports
each protocol separately. It also accepts arrays such as `[6, 17]` or
`["TCP", "UDP"]` when receiving several protocols.
