...
- fake DNS answers from a single packet, so fragmented **UDP** addressed to its endpoint is dropped with a
  ...
  is routed normally
- the zero-copy receive path wraps each incoming packet in a pooled descriptor sized from the shared
  reassembly budget in `ww/lwip/lwipopts.h`; exhausting it drops one packet with a rate-limited warning. Packet
  transforms may leave the buffer cursor unaligned, so that valid exceptional case is copied into an aligned
  lwIP-owned pbuf while the common aligned path remains zero-copy
