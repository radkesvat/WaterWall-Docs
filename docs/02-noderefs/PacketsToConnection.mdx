...
- packet emission is protected by an admission gate held through the previous-neighbour callback, and normal-line
  ...
  refused next-side task recycles its owned buffer and closes the PTC-owned normal line locally; it never calls a
  stopped neighbour
- netif output is always queued, even to the same worker, so no neighbouring callback runs while lwIP's non-recursive
  core lock is held. Fake-DNS lookup and response construction remain under that lock, but the completed response is
  returned to the outer packet handler and published only after it unlocks
- every generated TCP/UDP line is registered in a per-worker owner list until its one close path unlinks it. Stop first
  ...
  destroys the line state, and calls `lineDestroy()`. Configuration Stop waits for all worker drains; terminal worker
