...
description: "Reference documentation for the PacketsToConnection node."
---
<!--
Documentation version: 113
Sync note: Any change to this file must also be applied to WaterWall/tunnels/PacketsToConnection/description.md, and both files must keep the same documentation version.
-->
...
- the worker-local netifs inherit the core `misc.mtu`, so a response larger than it leaves as IPv4 fragments
  rather than as one oversized raw packet the packet topology could not carry
- packet emission is protected by an admission gate held through the previous-neighbour callback. Global shutdown
  runs a pre-stop pass for every tunnel before any ordinary `onStop()`: PTC closes and quiesces this gate there, so an
  already-admitted callback finishes before its neighbour can stop and a queued callback that runs later is cancelled
  without allocating a worker buffer. Netif output is always queued, even to the same worker, so no neighbouring
  callback runs while lwIP's non-recursive core lock is held
- Stop explicitly detaches every owned per-flow UDP PCB, wildcard UDP PCB, and TCP listener before removing its netif.
  ...
  still present; it never silently frees a pointer the tunnel could still retain
...
- the zero-copy receive path wraps each incoming packet in a pooled descriptor whose size is derived in
  ...
  budget and a fixed reserve. What makes it a bound is the per-PCB `TCP_OOSEQ_MAX_PBUFS`/`TCP_OOSEQ_MAX_BYTES`
  ceiling beside it, which stops one reordering peer from spending the whole pool; exhaustion past that drops
  one packet with a rate-limited warning
