...
Sync note: Any change to this file must also be applied to WaterWall/tunnels/PacketsToConnection/description.md and WaterWall-Docs/i18n/fa/docusaurus-plugin-content-docs/current/02-noderefs/PacketsToConnection.mdx, and all files must keep the same documentation version.
-->
...
- netif output is always queued, even to the same worker, so no neighbouring callback runs while lwIP's non-recursive
  core lock is held. Under lifecycle-v2, pending worker messages settle before component stop and destruction; quiesce
  closes the output admission gate so queued work running during quiescence cancels cleanly without calling into a
  stopped neighbour. Fake-DNS lookup, response construction, and submission to lwIP remain under the core lock; only
...
waits for sufficient acknowledgement progress. Admission is checked
before either queue takes ownership. A payload that would pass this bound is dropped, the
