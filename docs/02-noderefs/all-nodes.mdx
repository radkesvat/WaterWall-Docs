...
description: "Inventory of public configurable WaterWall nodes."
---
...
|---|---|---|
| `AuthenticationClient` | Maintains the authentication-control session and local users table for nodes that use its API. | Control-chain head leading to `AuthenticationServer`, often through `TcpConnector`. |
| `AuthenticationServer` | Terminal authentication-control backend that serves users and accepts client requests. | End of an `AuthenticationClient` control chain; it has no `next`. |
...
| `ConnectionToPackets` | Opens a connection line as a userspace lwIP TCP/UDP flow and emits raw IPv4 packets. | Connection-to-packet bridge after a destination-setting node such as `Socks5Server`. |
...
| `HttpProxyClient` | Sends HTTP/1.1 CONNECT or one configured HTTP request through an upstream proxy. | `source -> HttpProxyClient -> optional TlsClient -> TcpConnector`; target settings identify the origin. |
| `HttpProxyServer` | Forwards HTTP/1.x proxy requests and CONNECT through its configured next chain. | `TcpListener -> HttpProxyServer -> outbound chain`; explicit no-auth or AuthenticationClient. |
...
| `PacketSplitStream` | Splits one worker packet line into named upload and return branches. | Specialized packet-line helper using `settings.up` and `settings.down`. |
| `PacketsToConnection` | Converts packet traffic through lwIP into normal WaterWall connection lines. | Packet-to-connection bridge. |
| `PacketsToStream` | Carries raw IPv4 packet boundaries over a stream-facing line. | Paired with `StreamToPackets`. |
| `PingClient` | Carries local IPv4 packets in ICMP Echo Requests and consumes exact Echo Replies as acknowledgements. | Packet chains, paired with `PingServer`. |
| `PingServer` | Replies exactly to peer Echo Requests, decapsulates them once, and originates reverse data in new Echo Requests. | Packet chains, paired with `PingClient`. |
...
| `Socks5Server` | Accepts SOCKS5 client requests and forwards accepted traffic. | TCP-only after `TcpListener`; `udp: true` requires a preceding dynamic UDP provider, normally `TcpUdpListener`. |
| `SoftIpLimiter` | Limits how many source IPs may concurrently use the same early VLESS UUID or Trojan SHA224 identity. | After TLS/encryption termination and before `VlessServer`, `TrojanServer`, or a raw relay to one. |
...
| `StreamFragmenter` | Splits selected upstream TCP payloads into ordered fragments with configurable delays. | Middle of a TCP chain at the desired shaping point. |
...
| `UdpStatelessSocket` | Uses routing context for outbound datagrams and creates normal peer lines for inbound flows. | UDP packet edge, often with `WireGuardDevice`. |
...
| `WireGuardDevice` | Implements in-chain WireGuard crypto, peer state, and allowed-IP routing. | Between exactly one L3 packet side and one L4 transport side, often `UdpStatelessSocket`. |
...
- Rootless connection-to-packet bridge: `TcpListener -> Socks5Server -> ConnectionToPackets -> ...`, with
  `PacketsToConnection -> TcpUdpConnector` on the far side
