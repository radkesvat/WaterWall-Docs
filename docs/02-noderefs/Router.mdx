...
traffic. It sits in the middle of a chain, evaluates ordered rules, and sends
each connection to one selected branch. Metadata-only rules can select a branch
during `Init`; rules that need sniffing select one from the first upstream
payload.
...
Think of `Router` as an early selector when it can be, and a deferred branch
selector when it must inspect bytes:

- If no rule or root setting needs payload inspection, it evaluates the line's
  source/destination metadata, network, and credential facts during `Init` and
  initializes the selected branch immediately.
- A rule needing protocol detection, HTTP Host/:authority, TLS/QUIC SNI, or HTTP
  `Upgrade` waits for the first upstream payload and buffers those bytes while
  deciding.
- After a deferred rule wins, it initializes the selected branch and replays the
  buffered bytes to that branch.
...
At `Init`, Router clears stale observed-protocol bits while preserving separate
one-shot route-control metadata. This lets a protocol tunnel carry an already
negotiated destination through Router to its designated outbound adapter without
making that metadata available to `protocol` rules.

:::warning
Rules that need sniffing still need the client side to send first. For
server-first or handshake clients that need downstream establishment before
their first write, use only metadata-only rules (for example `network`, ports,
addresses, or credentials) and leave root `sniffing`, `protocol`, and
`attributes` out of that Router.
:::
...
Router initializes a branch during `Init` when its configured rules use only
metadata already on the line. If any configured rule or root setting needs
payload sniffing, Router instead waits for the first upstream payload; such a
configuration cannot initialize a server-first path until client data arrives.
