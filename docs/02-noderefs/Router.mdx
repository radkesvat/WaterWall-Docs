...
Sync note: Any change to this file must also be applied to WaterWall/tunnels/Router/description.md, and both files must keep the same documentation version.
-->
...
- It waits for the first upstream payload so it can inspect visible bytes when a
  rule needs protocol detection, HTTP Host, TLS SNI, or HTTP `Upgrade`.
...
connection to `next`; if `sniffing` is enabled it may still sniff the first
payload to populate destination-domain metadata before forwarding.
...
| `sniffing` | array | unset | Domain-sniffing modes. Supported values: `"http1"`, `"tls"`. |
| `sniff-even-if-domain-is-already-provided` | boolean | `false` | If `true`, Host/SNI sniffing may replace an existing destination domain. Use carefully. |
...
The old sniffing value `"http"` is invalid. Use `"http1"`.
...
| `destination-domain` | string or array | Destination domain, including sniffed HTTP Host or TLS SNI when enabled. Supports exact, wildcard, `*`, and `geosite:<list>`. |
...
| `username` | string or array | Exact case-sensitive authenticated username on the line. |
| `password` | string or array | Exact case-sensitive authenticated password on the line. |
...
For IP-only destinations, enable root `sniffing` so Router can read HTTP Host or
TLS SNI and store the observed domain as routing metadata.
...
The old value `"http"` is invalid. Use `"http1"`.
...
`username` and `password` match credentials already attached to the line by an
upstream authentication-capable tunnel.
...
or password, the corresponding condition does not match.
...
By default, Router does not overwrite a domain that already exists on
`dest_ctx`. That default protects destinations chosen by a previous tunnel or by
the config from being replaced by client-supplied Host/SNI.

Set `sniff-even-if-domain-is-already-provided: true` only when you deliberately
trust the application-layer Host/SNI more than the existing destination domain.

When a domain is sniffed from an IP-backed destination, Router stores it as
observed metadata while keeping the original IP endpoint. Upstream connectors
continue using the preserved IP when the address context is already resolved.

## First-Payload Buffering

Router buffers the first upstream bytes until classification is possible.

Detectors can ask for more bytes when the payload is a partial HTTP request, TLS
ClientHello, or BitTorrent handshake. The sniff window is bounded by the shared
protocol-sniff limit:
...
- It does not re-resolve Host/SNI domains sniffed later by Router.
...
or Router sniffed it from HTTP Host/TLS SNI. Enable root `sniffing` for this
case.

### Expecting `protocol` to need root `sniffing`

`protocol` detection is separate from Host/SNI domain sniffing. If a rule uses
