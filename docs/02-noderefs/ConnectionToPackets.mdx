...
Sync note: Any change to this file must also be applied to WaterWall/tunnels/ConnectionToPackets/description.md and WaterWall-Docs/i18n/fa/docusaurus-plugin-content-docs/current/02-noderefs/ConnectionToPackets.mdx, and all files must keep the same documentation version.
-->
...
- `onQuiesceRequest()` publishes stopping and closes new admission through the previous-side, next-side, and
  packet-classification/publication gates without waiting. `onQuiesceWait()` then proves already-admitted callbacks
  have returned. Under lifecycle-v2, emit, inject, fragment-purge, and startup packet-`Init` messages settle before
  component stop and destruction, so they can carry the tunnel pointer directly while cancellation releases their
  message-owned resources. After worker-owned lines drain, `onStop()` detaches every PCB, flow, netif, and staged
