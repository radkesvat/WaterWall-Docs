...
- `onPreStop()` publishes stopping and closes two reference-counted admission gates. The previous-side gate spans every
  ordinary normal-line callback, while the next-side gate spans packet-line `Init`/`Payload`. Closing waits for an
  already admitted callback to return and rejects every later callback before any neighbor can complete `onStop()`.
  `onStop()` then detaches every PCB, flow, netif and staged fragment under the lwIP core
  ...
  cannot dereference a released PCB. Every queued task enters the relevant gate, so a rejected task suppresses its data and lifecycle events
...
- If lwIP replays refused receive data from its real timer thread, the callback leaves the original pbuf retained,
  ...
  therefore remain worker-correct, including a data-plus-FIN replay. A required control enqueue refusal requests orderly
  process shutdown so Stop can discover and release the registered flow instead of silently losing a state transition.
