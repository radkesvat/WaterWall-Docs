...
- The closer writes until the peer has every accepted byte and then requests a TX-only shutdown. Its tuple stays
  ...
  ownership, and deadline, and records peer EOF without retiring. Only an actual transition to `FIN_WAIT_1` or
  `LAST_ACK` with `TF_CLOSEPEND` clear means the FIN was queued. At that point a separate `60` second peer-close
  deadline is armed. Closers are also bounded by a shared `16 MiB` byte budget and a `4096` object count; past any
  bound, the PCB is aborted exactly once rather than reporting a clean but truncated EOF or leaving an untracked PCB.
...
- `onStop()` publishes a stopping gate, then detaches every PCB, flow, netif and staged fragment under the lwIP core
  ...
  cannot dereference a released PCB. Every queued task rechecks the gate, so a task that observes it suppresses its data and lifecycle events
