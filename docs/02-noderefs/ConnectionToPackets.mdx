...
description: "Reference documentation for the ConnectionToPackets node."
---
<!--
Documentation version: 136
Sync note: Any change to this file must also be applied to WaterWall/tunnels/ConnectionToPackets/description.md, and both files must keep the same documentation version.
-->
...
The deadline restarts when staged fragments are bound to a flow, because that is when they first enter lwIP and lwIP's
own interval starts from that moment. It is a one-time reset on binding, not a refresh per fragment - lwIP's timer does
not refresh either.
...
TCP never fragments here: its send MSS is derived from the same MTU.
...
- A gracefully closed TCP flow leaves a tombstone in the flow registry so late FIN/ACK traffic still reaches the right
  worker and netif. It is a routing grace period of `2 * TCP_MSL`, not a model of the PCB's lifetime: `tcp_close()`
  moves the PCB through FIN_WAIT or LAST_ACK before TIME_WAIT even begins, so no fixed interval measured from the close
  can track it. A tombstone may therefore outlive its PCB or expire before it; neither loses a live flow.
...
- A local `Finish` or clean remote half-close first flushes accepted TCP bytes. Any remainder moves with the PCB to a
  node-owned drain, including while the active open is still in `SYN_SENT`; it writes until the peer has every byte and
  only then closes. Its tuple stays registered as *draining* - neither active nor a tombstone - so it cannot be evicted
  while this node still owns the PCB. Drains are best-effort within a shared `16 MiB` budget and a `30` second deadline.
  Resource refusal, deadline expiry, connect failure, or terminal write/close failure resets the TCP flow instead of
  reporting a clean but truncated EOF.
...
- `onStop()` publishes a stopping gate, then detaches every PCB, flow, netif and staged fragment under the lwIP core
  lock, and clears each live line's copy of its PCB pointer so a worker that has not drained yet cannot dereference a
  released PCB. Every queued task rechecks the gate, so a task that observes it suppresses its data and lifecycle events
  toward either neighbour, while close paths stay live so borrowed lines can still be released by their owners.
- The gate is not a strict quiescence boundary: a task that passed its check immediately before `onStop()` published the
  gate can still reach a neighbour afterwards. It is rechecked after every blocking section - waiting for the lwIP core
  lock is unbounded, since lwIP's timer thread holds it for whole sweeps - and again immediately before any callback
  toward a neighbour, which is as narrow as the window gets from inside this node. What the gate plus the core-locked
  cleanup does guarantee is that no PCB is touched after stop. Full callback quiescence would need a lifecycle counter
  in the framework rather than more one-time checks here.
...
*fragmented* datagrams are in flight, not of how many flows exist. It assumes 16 fragments per datagram, which covers an
8 KiB datagram at the smallest supported MTU (576), and `PBUF_POOL_SIZE` is held at three times that budget so a full
reassembly backlog cannot starve the receive path. Return packets are injected as reference pbufs rather than copied
into pool buffers, so one wire packet costs exactly one unit of that budget - which is the unit lwIP charges.

`PacketsToConnection`'s zero-copy receive path takes its wrapper-pool size from the same budget, since each fragment
lwIP retains pins one wrapper.
...
These pools are static. With the documented defaults a Linux release build has roughly **9.5 MB total BSS**, about an
**8.5 MB increase** over the earlier development-sized pools' roughly 1.0 MB. Lower the cache variables to trade
