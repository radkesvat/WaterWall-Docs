...
- The profile-aware format remains Reality v2 but replaces the earlier unpublished fixed-prefix v2 layout. Kind-aware AAD changes every v2 record tag, so upgrade both peers together; there is no old-layout or v1 fallback.
