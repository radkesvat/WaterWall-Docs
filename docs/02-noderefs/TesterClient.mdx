...
Sync note: Any change to this file must also be applied to WaterWall/tunnels/TesterClient/description.md and WaterWall-Docs/i18n/fa/docusaurus-plugin-content-docs/current/02-noderefs/TesterClient.mdx, and all files must keep the same documentation version.
-->
...
For response packets, source and destination addresses are reversed automatically. For `tcp`, `udp`, and `icmp`, deterministic transport headers are generated and verified. TCP and UDP use source port `40123` and destination port `40234` for requests, reversed for responses. With `worker-affine-flow` enabled the request source port is instead the first port at or above `40123` whose inner flow selects the current worker, so both nodes must share `source-ip`, `dest-ip`, `transport` and the setting itself.
...
`TesterClient` supports at most `254` workers and uses a `30000 ms` watchdog for each worker.
