...
Sync note: Any change to this file must also be applied to WaterWall/tunnels/ConnectionFisherServer/description.md and WaterWall-Docs/i18n/fa/docusaurus-plugin-content-docs/current/02-noderefs/ConnectionFisherServer.mdx, and all files must keep the same documentation version.
-->
...
The continuing path is published before calling next Init. Transport Est is
forwarded promptly once even if next emits it synchronously from Init.
Later payload forwards normally without waiting for Est.
