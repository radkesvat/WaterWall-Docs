...
| --- | --- | --- |
| `sni` | not set | Case-insensitive exact SNI match. Mismatch or absent SNI routes to fallback when configured; otherwise rejects the handshake. |
...
| `fallback-node-name` | not set | Fallback branch for clearly non-TLS input or mismatched/absent configured SNI before TLS commit. Aliases: `fallback-node`, `fallback`. |
...
`sni` can be combined with fallback to route mismatched or absent SNI to a real TLS listener before local TLS output.
...
- bytes that start like TLS handshake records, such as `16 03`, are parsed by OpenSSL; when `sni` and fallback are configured, the early ClientHello callback routes mismatched or absent SNI to fallback
...
Malformed ClientHello/SNI input rejected before routing stays on the TLS error path. Incomplete or slow input waits for the handshake deadline; the fallback server handles TLS negotiation and errors after an SNI fallback selection.
...
- When combining `sni` with fallback, point fallback to a TLS listener that can handle the original ClientHello.
