...
Sync note: Any change to this file must also be applied to WaterWall/tunnels/ReverseServer/description.md and WaterWall-Docs/i18n/fa/docusaurus-plugin-content-docs/current/02-noderefs/ReverseServer.mdx, and all files must keep the same documentation version.
-->
...
`ReverseServer` must connect directly to the `Bridge` on its next side:
`ReverseServer -> Bridge`, with no intervening node. The companion client-side
rule is `Bridge -> ReverseClient`, also with no intervening node. These are
chain-design requirements that users must follow; the code does not automatically
enforce either adjacency rule.
...
| `next` | string | Required in practical Bridge layouts. Must name the directly adjacent Bridge that leads to local/user traffic; no intervening node is allowed. |
...
```text
65535 * max(1, ceil(L / 32768)) bytes per waiting half
```

Here `L` is the line pool waiting-budget basis (64 KiB in S1/S2, 1 MiB in higher profiles),
independent of ordinary buffer sizes and the splice limit. The limit is 131,070
bytes in S1/S2 and 2,097,120 bytes in higher profiles. Validate the handshake and try
available pairing before applying this waiting-only bound.
...
`ReverseServer` does not prepend protocol bytes after handshake handling and does not need left padding.
