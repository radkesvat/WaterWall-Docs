...
- Reads the complete 17-byte intro: one exact role byte and one 128-bit pair ID.
...
until all 17 bytes are available, then consumes the intro sent by
`HalfDuplexClient`.
...
4. strips the complete 17-byte upload intro from the first upload payload
...
The pair ID is routing metadata, not authentication or transport protection;
configure security nodes when those properties are required.
...
| Value | Size |
| --- | --- |
| maximum waiting upload buffer | `131070 * max(1, ceil(L / 32768))` bytes |
| internal intro | `17 bytes` (one role byte plus a fresh 128-bit pair ID) |
| `required_padding_left` | `0 bytes` |

`L` is the line pool waiting-budget basis (64 KiB in S1/S2, 1 MiB in higher profiles),
independent of ordinary buffer sizes and the splice limit. The limit is 262,140 bytes
in S1/S2 and 4,194,240 bytes in higher profiles. It applies to initial publication and
later appends; already-paired data forwards directly.
...
- The tunnel depends on the internal 17-byte intro generated by
  `HalfDuplexClient`.
- Duplicate waiting upload or download pair IDs are rejected and the duplicate
  half-line is closed.
