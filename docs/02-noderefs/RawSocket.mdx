...
Sync note: Any change to this file must also be applied to WaterWall/tunnels/RawSocket/description.md and WaterWall-Docs/i18n/fa/docusaurus-plugin-content-docs/current/02-noderefs/RawSocket.mdx, and all files must keep the same documentation version.
-->
...
before it restores the inner packet toward TunDevice. Plain packets from
TunDevice travel downstream as fresh type-8 Echo Requests for raw injection;
they are not inserted into an unrelated Echo Reply.
