...
description: "HTTP forwarding and CONNECT through a configured WaterWall chain."
---
...
Fixed limits are an 8192-byte start line, 128 header fields, a 1024-byte chunk-size/extension line, a 16384-byte/64-field trailer section, and 32 informational responses per exchange. Pending content is counted across both directions, including queued rewritten headers. The working budget `P = max-pending-bytes` covers four coalesced stream/output buffers with allocation charge at most `4 * P`. Each direction also permits one already-delivered remainder of at most `D = 2 MiB` bytes. This fixed per-direction headroom is independent of pool sizes, RAM profile and splice settings. Total retained logical bytes are at most `P + 2 * D`. A single delivery is limited to `P + D`; an immediately drainable prefix may consume working space before the remainder is retained. Each remainder uses ordinary best-fit storage with payload capacity at most `max(small capacity, 2 * D)` plus cache-line rounding, full pool padding, the sbuf header, and aligned-allocation overhead. Deferred input participates in pressure and drains FIFO before newer bytes; header parsing/rewrite scratch and credentials have separate bounded storage. Input is processed in streaming slices, so the pending limit is not a body-size limit. Pressure pauses production and resumes below the low watermark; overflow closes the exchange.
