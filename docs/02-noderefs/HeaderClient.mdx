...
Sync note: Any change to this file must also be applied to WaterWall/tunnels/HeaderClient/description.md and WaterWall-Docs/i18n/fa/docusaurus-plugin-content-docs/current/02-noderefs/HeaderClient.mdx, and all files must keep the same documentation version.
-->
...
For numeric and source-port modes, the first upstream payload is rewritten from:
...
If the first upstream payload is empty, the tunnel still sends the configured
header on that first payload callback.
...
The node declares `required_padding_left = 108` because it prepends the largest
supported header with `sbufShiftLeft()`.
...
The one-time header is written only into advertised real left padding, ahead of any existing prefix. The application body stays in its private pipe; subsequent upstream and all downstream payloads pass through unchanged. HeaderClient support does not imply HeaderServer support: a chain containing the unchanged HeaderServer remains in ordinary mode.
