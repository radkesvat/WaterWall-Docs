...
Sync note: Any change to this file must also be applied to WaterWall/tunnels/TrojanClient/description.md and WaterWall-Docs/i18n/fa/docusaurus-plugin-content-docs/current/02-noderefs/TrojanClient.mdx, and all files must keep the same documentation version.
-->
...
2. forwards downstream `Est` to the previous node
3. releases the establishment barrier after that callback returns
...
larger than 8192 bytes, malformed address headers, bad CRLF separators, or carrier input buffering above 1,057,031 bytes
close the affected association.

The client queues up to 1 MiB and 1,024 buffers of upstream application data while waiting for establishment or Resume.
