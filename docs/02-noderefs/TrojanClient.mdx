...
`TrojanClient` reserves enough padding for its 68-byte UDP association request and
the largest 263-byte datagram header, combined on the first payload:

```text
required_padding_left = 331
```

The datagram header covers `ATYP + domain length + 255-byte domain + port + payload length + CRLF`.
The same budget also covers the largest 320-byte TCP request.
