...
When local resolution is enabled, `TrojanClient` inserts an internal `DomainResolver` after an internal destination setup
step. This works for both fixed JSON targets and destination-context targets. Literal IP addresses are not resolved.
...
On upstream `Init`, the internal setup path prepares `line->dest_ctx` from the configured target settings and optional DNS
resolution. `TrojanClient` then forwards the same line to the next node.
