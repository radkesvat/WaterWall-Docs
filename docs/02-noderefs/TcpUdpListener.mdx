...
Both internal listener nodes retain their capability flags. The TCP branch can use its existing splice receive/send path only when the entire expanded chain is eligible; UDP receives stay ordinary. Protocol selection is unchanged.
