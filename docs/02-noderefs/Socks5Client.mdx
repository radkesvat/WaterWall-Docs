...
`Socks5Client` always creates an internal target setup tunnel. That setup step writes the final SOCKS target into
`line->dest_ctx` before the SOCKS protocol code runs, and it also stores the selected protocol for the real client line.

When `domain-strategy` enables local resolution, the client also inserts an internal `DomainResolver` between the setup
step and `Socks5Client`. Resolution applies to fixed domain targets and to targets copied from `dest_context->address`.
Literal IP addresses are left unchanged.
