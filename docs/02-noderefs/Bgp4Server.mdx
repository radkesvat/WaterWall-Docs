...
| Maximum retained read bytes | `max(65536, 2 * L) + 65553` | `L` is the line pool large payload capacity; complete frames drain before the next delivery. |
| Required left padding | `19` bytes | Frame marker, length, and type. |

Large deliveries are split into wire-valid frames, with at most 65,534 ordinary
payload bytes per frame. The first client OPEN's actual synthetic prefix reduces
its payload room; exactly one OPEN is emitted. Each delivery is encoded into one
aggregate buffer. Complete decoded frames are combined into one onward callback,
so a re-entrant Pause cannot be followed by another send from the same batch.
Fragmented headers/bodies and coalesced complete frames with partial tails are
supported. Malformed records and unrepresentable allocations close the line.
