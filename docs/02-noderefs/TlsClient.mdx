...
Sync note: Any change to this file must also be applied to WaterWall/tunnels/TlsClient/description.md and WaterWall-Docs/i18n/fa/docusaurus-plugin-content-docs/current/02-noderefs/TlsClient.mdx, and all files must keep the same documentation version.
-->
...
| `fragment` | absent | Optional private StreamFragmenter settings for outgoing TLS bytes. |
...
Downstream transport `Est` is forwarded immediately and once in both ordinary and takeover modes. Payload may arrive
after adjacent Init, before either transport Est or TLS completion. Plaintext waiting for TLS readiness or older reentrant
output is bounded to 2 MiB and 1,024 buffers, including its active retained head. Equality is accepted; transactional
admission failure closes the borrowed line through its owner and releases retained buffers. Handshake completion and
Resume release this FIFO through `SSL_write()` only while wire output is not paused, stopping before the next record
when a callback pauses output. New input cannot overtake the retained head. Ciphertext-shaping limits remain separate.
...
Takeover registers a dedicated owner completion callback with `tlsclientTunnelEnableHandshakeTakeover(t, owner, callback)`.
`TlsClient` frames takeover input one TLS record at a time and stops at the exact record that completes the handshake,
leaving later records outside the SSL object. It publishes completion before invoking the callback, retains the exact line,
and rechecks life and TLS state afterwards. Neither completion nor later Reality activation emits another `Est`.
...
| API | Purpose |
| --- | --- |
| `tlsclientTunnelEnableHandshakeTakeover()` | Registers the owner and its handshake-ready callback and enables takeover. |
