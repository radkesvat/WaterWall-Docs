...
Optional defaults apply only when their keys are absent. Present `verify`, `x25519mlkem768`, and `verbose` values must be JSON booleans, and a present `ech-sni-trick` must be a non-empty JSON string. A wrong type is a startup error rather than a request to use the default.
