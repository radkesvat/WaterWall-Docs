...
extension. Present `verify`, `x25519mlkem768`, and `verbose` values must be JSON booleans, and a present `ech-sni-trick`
must be a non-empty JSON string. A wrong type is a startup error rather than a request to use the default.
...
- Offering both `h2` and `http/1.1` is unsafe when the preceding `HttpClient` is committed to one version. The server may
  choose either protocol, but `HttpClient` cannot learn which one was selected. It can then send HTTP/1.1 over a
  connection negotiated as `h2`, or send HTTP/2 over one negotiated as `http/1.1`.

Only offer multiple protocols when the preceding application layer can safely handle every possible selection without
ALPN-selection feedback from `TlsClient`. Changing `alpns` also changes a fingerprint-relevant part of the otherwise
Chrome-like ClientHello.
...
- the embedded fake ClientHello uses the same configured `alpns` list and order
...
The generated ClientHello uses this tunnel instance's configured behavior, including `alpns`, `x25519mlkem768`, and any
configured `ech-sni-trick`.
...
- Do not offer application protocols that the preceding node cannot actually speak. `TlsClient` does not communicate the
  server's ALPN choice back to that node.
