...
Every rewrite operation with `preserve-tcp-bitflags` appends exactly one byte.
`IpManipulator` does not compare the enlarged packet with `misc.mtu`,
`GLOBAL_MTU_SIZE`, `kMaxAllowedPacketLength`, or the path MTU, and it does not
fragment the packet or reduce TCP MSS. It only checks whether the current
buffer has room for one more byte. If the buffer has no room, the packet is
dropped and a warning is logged. If the buffer does have room, the enlarged
packet is forwarded; for example, a 1500-byte input becomes 1501 bytes and can
then be discarded by `RawSocket`, `TunDevice`, another packet node, or the
physical network.

For each direction that has TCP-bit rewrite actions, use this budget:
...
For example, with upstream-only actions in a direct
`TunDevice -> IpManipulator -> RawSocket` path limited to 1500 bytes, configure
the TUN input MTU to at most `1499`. Setting `misc.mtu` to `1499` achieves that
only when `TunDevice.settings.device-mtu` is omitted; if `device-mtu` is set
explicitly, set it to `1499` or lower as well.

If downstream or both-direction TCP-bit actions are configured, budget the
input to those rewrite directions separately. Lowering the local TUN MTU does
not constrain packets that arrive from `RawSocket`. Also subtract overhead
added by any other size-growing node or by another `IpManipulator` rewrite.
...
Oversized crafted TCP packets are segmented before port ghost is applied. Each
emitted segment receives exactly one trailer, and the segmentation payload
budget reserves space for that trailer so the result stays within
`GLOBAL_MTU_SIZE`.
