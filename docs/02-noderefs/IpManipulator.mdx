...
transport pair: it restores the real protocol, completes any requested checksum work under
...
`mtu`. Every segment gets independently derived live and saved
...
`mtu`, then final packet duplication is applied to that segment.
...
assembled packet, or a decoy built from it, that exceeds `mtu` is
