...
Whenever the tunnel changes the protocol field, it recalculates only the IPv4
header checksum with `calcIpv4HeaderChecksum()`. Transport bytes and transport
checksums remain untouched, and protocol swap does not create or clear a
full-recalculation request.
