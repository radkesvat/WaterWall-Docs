...
Because rewritten packets grow by one byte, reduce `misc.mtu` in `core.json` by
`1` when the current MTU is already `1500` or the maximum value supported by the
network card or packet path. For example, use `1499` instead of `1500`.
