...
Sync note: Any change to this file must also be applied to WaterWall/tunnels/PacketSplitStream/description.md and WaterWall-Docs/i18n/fa/docusaurus-plugin-content-docs/current/02-noderefs/PacketSplitStream.mdx, and all files must keep the same documentation version.
-->
...
During chain construction, `PacketSplitStream` binds its configured `down` branch as its chained `next` tunnel, allowing downstream return packets from that branch. It does not prepend, rewrite, or consume payload buffers.
