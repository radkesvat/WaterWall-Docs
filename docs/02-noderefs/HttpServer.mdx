...
`HttpClient` supplies a fresh 128-bit CSPRNG pairing ID made from two `fastRand64()` values (16 bytes from the thread's ChaCha20 stream) and formatted as 32 lowercase hexadecimal characters. The server uses it as ordinary routing metadata to match the upload and download halves. Add `TlsServer`, `EncryptionServer`, or another security node if the deployment needs protected transport.
...
The split waiting limit is `131070 * max(1, ceil(L / 32768))` bytes, where `L`
is the line pool waiting-budget basis (64 KiB in S1/S2, 1 MiB in higher profiles),
independent of ordinary buffer sizes and the splice limit. Close occurs strictly above
this limit: 262,140 bytes in S1/S2 and 4,194,240 bytes in higher profiles. Header parsing
and available pairing run first; coalesced body bytes do not count toward the
header-size limit. Paired bodies retain normal HTTP framing validation.
