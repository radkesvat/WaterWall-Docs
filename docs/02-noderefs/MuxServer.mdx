...
| `parent-buffer-limit` | integer | `50331648` | Per-parent queue-capacity budget for incoming assembly plus attached child queues. Beneficial compaction precedes largest-child/oldest-tie shedding; unrelieved pressure closes this parent. Set to `0` to disable the aggregate budget. |
...
| parent upstream `Pause` / `Resume` | Stops all parent output immediately / drains its FIFO. Child producers are gated only at the configured retained-charge threshold. |
