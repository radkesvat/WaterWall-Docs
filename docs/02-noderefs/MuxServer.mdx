...
| `parent-buffer-limit` | integer | `8388608` | Per-parent budget for queued child-destined data. When reached, the actual largest queued child is closed; equal-size ties prefer the least recently active child. Set to `0` to disable the aggregate budget. |
