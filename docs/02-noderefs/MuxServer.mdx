...
| `parent-buffer-limit` | integer | `33554432` | Per-parent budget for queued child-destined data. When reached, the actual largest queued child is closed; equal-size ties prefer the oldest attached child. Set to `0` to disable the aggregate budget. |
...
- If total queued child data reaches `parent-buffer-limit`, the child with the
  actual largest queue is closed. Equal-size ties prefer the oldest attached
  child.
