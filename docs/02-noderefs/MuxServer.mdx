...
count, child read-pause count, and child write-pause count. The legacy
`parent-line-read-paused` field remains present as `no` for compatibility.
...
the parent, releases its queued-byte accounting, destroys the child's MUX state,
sends upstream `Finish` to `next`, and destroys the child line if it is still
alive.
