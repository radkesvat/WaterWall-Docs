...
| `parent-buffer-limit` | integer | `50331648` | Per-parent retained-allocation-charge budget for child queues. When reached, the child with the largest retained charge is closed; equal-size ties prefer the oldest attached child. Set to `0` to disable the aggregate budget. |
