...
Sync note: Any change to this file must also be applied to WaterWall/tunnels/VlessClient/description.md, and both files must keep the same documentation version.
-->
...
When local resolution is enabled, `VlessClient` inserts an internal `DomainResolver` into its chain. It also inserts an
internal domain-setup tunnel before itself to copy or overwrite the line destination context before DNS resolution. This
works for both fixed JSON targets and `"dest_context->address"`.

Literal IP addresses are not resolved.
...
On upstream `Init`, the internal setup path prepares the target context and optional DNS resolution. `VlessClient` then
