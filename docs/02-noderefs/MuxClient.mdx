...
Sync note: Any change to this file must also be applied to WaterWall/tunnels/MuxClient/description.md and WaterWall-Docs/i18n/fa/docusaurus-plugin-content-docs/current/02-noderefs/MuxClient.mdx, and all files must keep the same documentation version.
-->
...
- `max-children` `(integer, optional)`
  ...
  positive. This live cap is separate from counter mode's cumulative
  `connection-capacity`.
...
Child response dispatch uses a per-parent hash index and remains average O(1).
If `MuxServer` rejects one fresh Open with `Close(cid)`, `MuxClient` finishes only
that matching borrowed child; its parent and unrelated siblings continue.
