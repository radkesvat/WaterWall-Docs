...
- The native-sized profile-aware format remains Reality v2 but replaces the earlier unpublished 12-byte-prefix application layout. Upgrade both peers together; the secure path never falls back to an older layout or v1. The obsolete `max-frame-size` setting is rejected rather than ignored.
