...
- `PacketSender` is direction-specific: downstream absorbs the legitimate close
  from `UdpConnector`, while upstream remains fatal. `PacketReceiver` is also an
  intentional terminal absorber.
