...
`LARGE_BUFFER_SIZE_RAM_HIGH` limit. The read loop checks out a splice wrapper with
its private pipe already initialized by the pool, then performs the socket-to-pipe
splice in nonblocking mode
**before invoking the Payload callback**.
...
pipe creation fails, checkout recycles the unused wrapper internally and returns
`NULL` with `errno`; the read loop falls back to an ordinary read for that delivery.
No socket bytes have been consumed at that point. Splice mode stays enabled, so
later reads may create or reuse a pipe.
...
remaining stream bytes stay queued for later reads. A TCP splice fallback reuses
its existing availability result without querying again.
