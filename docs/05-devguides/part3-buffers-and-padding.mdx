...
transfer size, capped by the loop's large-buffer size and the 512 KiB
...
branch, then verifies that the private pipe is empty. WIO cleanup, BufferQueue
destruction, `lineReuseBuffer()`, and `reuseBuffer()` share this behavior without
caller-side splice cleanup.

A wrapper claiming zero length must already have an empty pipe; the pool
verifies kernel emptiness with `FIONREAD` in Release and
with pooling bypassed. Resetting sbuf fields never empties a pipe. Normal reuse
...
Nonempty splice entries require `kSbufFlagSplicePiped`; empty unused wrappers
are valid. Splice lifetime metadata must remain null. Ordinary Debug insertion
...
sbuf_t *medium = bufferpoolGetMediumBuffer(pool); // fixed 64 KiB helper buffer
...
order. Their payload capacities, excluding reserved left padding, are:

| Tier | Capacity | Use |
| --- | --- | --- |
| Small | 4 KiB | Small ordinary payloads and control messages |
| Medium | 64 KiB in every memory profile | Helper storage for framing and retained queues, including paused Mux children |
| Large | 4 KiB in S1/S2; 512 KiB in M1/M2/L1/L2 | Ordinary event-loop read buffers and larger working payloads |
| Splice | 32 bytes of control storage | Explicit splice wrappers; logical body size is independent of allocation size |

Medium capacity is fixed; its local and master cache counts follow the memory
profile, just like the other tiers. Ordinary event-loop reads explicitly request
large buffers and do not select medium buffers. A node may use medium buffers for
its own work or retention. The tier name does not imply size ordering in low-memory
profiles, where a medium buffer is larger than a large buffer.
