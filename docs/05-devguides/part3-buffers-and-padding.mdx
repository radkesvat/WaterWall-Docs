...
Outgoing encoded Open, Data, Close, and flow-control frames share one lazy
`buffer_queue_t` per parent, separate from incoming `pending_child_data` queues.
Parent Pause stops parent-bound Payload immediately; retained output outlives its
originating child. FIFO admission and Open/flow-state publication precede any
forwarding or child notification that can re-enter Mux.

`parent-write-buffer-pause-threshold` defaults to 8 MiB of retained allocation
charge, and `parent-write-buffer-limit` defaults to 16 MiB. Both accept positive
integers up to `INT_MAX`; the final threshold must be below the limit. Omitted
settings independently use their defaults. Each parent gets its own budget.
Admission rejects a projected charge strictly greater than the hard limit before
queue ownership transfers; equality is allowed. Reservation refusal and overflow
close that parent locally. Small controls use fitting ordinary pooled buffers
with onward padding. A writable, empty, inactive FIFO permits direct delivery
without queue allocation, including buffers larger than the retention budget.

The running charge accounts for the exact retained pointer returned by insertion;
Debug replacement preserves ordinary allocation geometry. Pop subtracts the
charge before transferring ownership. Parent Resume does not reset accounting.
Child-wide callbacks occur only on a real local gate transition, using referenced,
attachment-checked snapshots. Release waits for empty plus writable and preserves
peer-flow pressure. Parent loss and owner Stop discard outgoing backlog; incoming
detached-drain accounting and `parent-buffer-limit` retain their existing meanings.
These outgoing thresholds measure allocation charge; incoming child FlowPause and
FlowResume thresholds still measure logical payload bytes.
...
a hard bound—currently the Mux incoming child/parent/detached limits and outgoing parent FIFO—must keep
...
incoming-child payload-based pause and resume thresholds.
...
alignment overhead, and pool/queue storage. Making accounting splice-aware does
not make Mux splice-capable.
