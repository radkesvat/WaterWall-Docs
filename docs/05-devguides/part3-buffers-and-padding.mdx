...
Linux TUN GSO reserves both the predicted actual allocation charge and one
packet slot before creating each segment. Its credit stays with the existing
reserved worker batch while a private destination-worker callback completes
the TCP checksum and calls normal TunDevice delivery. Release the credit after
delivery, or exactly once on refusal or cancellation; posting the message is
not settlement. This uses the existing buffer and worker admission path without
a new per-packet lock or packet-line flag.
