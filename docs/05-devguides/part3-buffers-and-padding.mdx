...
| `parent-write-buffer-pause-threshold` | **32 MiB charge** | One outgoing parent FIFO. Reaching it pauses child producers while the FIFO drains. |
| `parent-write-buffer-resume-threshold` | **28 MiB charge** | Release the outgoing producer gate at or below this charge while transport is writable. |
...
`parent-write-buffer-pause-threshold` defaults to **32 MiB**,
`parent-write-buffer-resume-threshold` to **28 MiB**, and
...
An omitted resume derives as `floor(effective_pause * 7 / 8)` using a wide
...
interrupts Resume; a borrowed parent-local cursor advances before each offered
release and is repaired on unlink. Snapshot references protect mutation; startup
completion reconciles both directions. Peer-flow and terminal-close reasons stay
...
the unchanged 24 MiB child and 48 MiB aggregate receive budgets still apply.
