...
pipe syscall. Descriptor zero is valid. `sbufSpliceInitPipe(buf, preferred_capacity)` lazily creates a
...
transfer size, capped by the loop's large-buffer size and the 1 MiB
...
| Medium | 32 KiB in S1/S2; 64 KiB in M1/M2/L1/L2 | Helper storage for framing and retained queues, including paused Mux children |
...
medium and 1 MiB large buffers. Ordinary event-loop reads explicitly request
large buffers. Medium buffers remain helper storage for nodes and retained
queues. Mux uses the large tier when a paused frame exceeds medium capacity,
