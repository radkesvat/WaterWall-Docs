...
StreamFragmenter's optional ClientHello inspection consumes only the bounded
initial wire prefix through these range helpers, including private-pipe and
resident-prefix-plus-pipe input. Its ordinary accumulator retains the original
record headers for byte-identical replay on unsupported framing or deadline
expiry. The replacement is reserved alongside the accumulator before record
headers are rewritten; an already-admitted opaque suffix remains in the normal
FIFO budget. Later opaque splice wrappers retain their direct forwarding path.
...
`sbufTryCreateWithPadding()` makes a separate ordinary `sbuf_t` with the same
capacity and alignment rules as `sbufCreateWithPadding()`, returning `NULL`
for unrepresentable geometry or allocation failure. It bypasses the pool's
fail-fast refill path, so optional startup storage can fall back without
changing ordinary pooled allocation behavior. Destroy this direct buffer with
`sbufDestroy()` after its last user has stopped.
...
into one ordinary buffer. It advertises 17 bytes of headroom and reuses ordinary input
when those bytes fit. Insufficient headroom uses a best-fit allocation with full onward padding. Consume splice input
...
eligible application payload in **one ordinary buffer**. Reuse a locally owned ordinary
body when the complete request and any first UDP frame header fit its advertised left
padding. Otherwise allocate best-fit ordinary storage and append the source with the shared exact range
...
Keep full onward padding in new allocations. TrojanClient advertises **331** bytes:
the 68-byte UDP association request plus the largest 263-byte datagram header; its largest
TCP request is 320 bytes. VlessClient advertises **280** bytes: its largest 278-byte request
plus the two-byte UDP length prefix. UDP first output contains the initial request, one
datagram frame header and that complete body. Later datagrams still prepend only their
own frame header (at most 263 bytes for Trojan and two for VLESS). Header-only timeout
...
following frames untouched. Complete accepted frames continue after Pause. UDP replies prepend a two-byte length prefix;
insufficient headroom uses shared movement, preserving any resident prefix and private body. Empty/oversized local replies
drop without closing the backend. Send the `00 00` response header exactly once, combined with the complete first nonempty
protected reply in one ordinary buffer. VlessServer advertises four padding bytes for the response header plus the UDP
length prefix on the first datagram. Reuse ordinary input in place when its remaining headroom fits the response header.
Otherwise allocate a best-fit ordinary buffer with full onward pool padding and copy through shared range helpers,
materializing any private-pipe body. For UDP the combined buffer also contains the datagram length prefix. Later replies retain their
normal representation. Est, Resume and empty input must not generate a standalone response header; no timeout or Finish
flush emits one. Publish the response-sent latch before the combined handoff to preserve FIFO under reentry.
Est remains prompt and independent of reply availability and output Pause.
...
Linux TUN GSO reserves storage before transferring a whole aggregate to its
flow's worker. Its session budget is 8 MiB of allocation charge and 512 packet
slots. Each GSO work item charges its actual aggregate buffer allocation, its
work context, and one maximum output-buffer allocation allowance; it reserves
two packet slots for the retained aggregate and active output. The worker
allocates and delivers one independent segment at a time. The output allowance
stays reserved through every segment's checksum completion and normal TunDevice
delivery, so the worker never waits for reader budget capacity. Downstream
retention follows ordinary packet-chain ownership after delivery.

A later startup configuration can increase shared worker-pool padding. Before
allocating each segment, the worker checks the current best-fit charge against
its prepaid allowance. If that tier no longer fits, it creates an exact-sized
ordinary buffer with this chain's original sufficient padding instead.

Ordinary batches use the same per-worker FIFO and reserve their actual buffer
allocation charges and packet slots before posting. Reader drain staging and
fragment storage retain their separate existing bounds; session and FIFO-message
metadata are bounded separately by the packet limit. The 8 MiB admission charge
is not a total process-memory limit. Budget exhaustion makes
the reader wait, retaining at most one unadmitted aggregate in its current
scratch buffer. Successful GSO admission transfers that buffer itself and
replaces reader scratch; it does not copy the aggregate payload on the reader.
Release each work reservation exactly once after completion, refusal,
cancellation, or stale-generation discard. Posting a work item is not
settlement, and cancellation must not borrow a device or worker buffer pool.
