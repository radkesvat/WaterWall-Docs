...
- **Auxiliary threads** must post work to an event worker (e.g. via `wloopPostControlEvent` / worker message queue) and must **never borrow a worker-local pool** directly.
